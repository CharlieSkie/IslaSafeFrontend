    ...
    <main className="relative grid min-h-[100svh] overflow-x-hidden overflow-y-auto bg-[#050810] text-slate-100 lg:h-[100svh] lg:grid-cols-[minmax(0,1.1fr)_minmax(480px,0.9fr)] lg:overflow-hidden">
      ...
      <section className="relative flex min-h-[100svh] items-center justify-center px-5 pb-20 pt-8 sm:px-8 sm:pb-20 sm:pt-12 lg:h-[100svh] lg:overflow-y-auto">
        <div className="w-full max-w-md"><div className="mb-8 flex items-center gap-3 lg:hidden"><span className="grid size-10 place-items-center rounded-xl border border-indigo-300/30 bg-gradient-to-br from-indigo-500 via-blue-600 to-cyan-500"><Activity className="size-5 text-white" /></span><span><span className="block font-display text-lg font-bold text-white">IslaSafe</span><span className="block text-[9px] font-bold tracking-[0.18em] text-slate-500">MDRRMO OPERATIONS</span></span></div>
          <div className={`panel p-5 sm:p-7 ${showPasswordRecovery ? '' : 'flex min-h-[620px] flex-col'}`}>
            {showPasswordRecovery ? <ForgotPasswordForm initialEmail={email} onBackToSignIn={() => setShowPasswordRecovery(false)} /> : <>
              <div className="mb-7"><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-indigo-300">Secure admin portal</p><h2 className="mt-2 font-display text-2xl font-bold text-white">{isSignIn ? 'Welcome back' : 'Create your account'}</h2><p className="mt-2 text-sm leading-6 text-slate-500">{isSignIn ? 'Sign in to continue to the operations dashboard.' : 'Register an administrator account for IslaSafe.'}</p></div>
              ...
              <form className="flex min-h-0 flex-1 flex-col justify-between gap-4" onSubmit={submit}>
                {!isSignIn && <AuthInput autoComplete="name" icon={UserRound} label="Full name" placeholder="Admin MDRRMO" required type="text" />}
                <AuthInput autoComplete="email" icon={Mail} label="Email address" onChange={(event) => setEmail(event.target.value)} placeholder="name@cpg.gov.ph" required type="email" value={email} />
                <LoginPasswordField isSignIn={isSignIn} onToggle={() => setShowPassword((current) => !current)} showPassword={showPassword} />
                ...
                {isSignIn && <div className="flex items-center justify-between gap-3"><label className="flex cursor-pointer items-center gap-2 text-xs text-slate-400"><input className="size-3.5 rounded border-white/20 bg-white/5 accent-indigo-500" type="checkbox" /> Remember me</label><button className="text-xs font-semibold text-indigo-300 hover:text-indigo-200" onClick={() => setShowPasswordRecovery(true)} type="button">Forgot password?</button></div>}
                <button className="action-button-primary mt-2 h-11 w-full text-sm" type="submit">{isSignIn ? 'Sign in to dashboard' : 'Create account and continue'}<ArrowRight className="size-4" /></button>
              </form>
